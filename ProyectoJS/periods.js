async function showPeriods(){
    ...
    const periodsHTML = document.getElementById("mainperiods");
    periodsHTML.innerHTML = "";// assure that is empty
    periodsHTML.innerHTML += "<div class='h2 text-center mt-5'>periods</div>"
    const periods = await load("periods");
    for(period of periods){
        if((period["id"]-1)%2 === 0){//the rows that we create only have 1 spaces
